...
import { deleteServerEmoji } from "@/lib/store";

type Params = {
  params: Promise<{ serverId: string; emojiId: string }>;
};
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const result = await deleteServerEmoji(serverId, emojiId, authenticatedUserId);
    await deleteEmojiFileByUrl(result.deletedUrl);
    return NextResponse.json({ ok: true });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Falha ao remover emoji." },
